...
firewall() {
  local port="${1:-1194}" docker_network="$(ip -o addr show dev eth0 |
    awk '$3 == "inet" {print $4}')" \
...
vpn() {
  local server="$1" user="$2" pass="$3" port="${4:-1194}" proto=${5:-udp} \
    i pem="$(\ls $dir/*.pem 2>&-)"
...
if [[ $# -ge 1 && -x $(which $1 2>&-) ]]; then
  ...
  [[ -e $conf ]] || {
    echo "ERROR: VPN not configured!"
    sleep 120
  }
  [[ -e $cert ]] || grep -Eq '^ *(<ca>|ca +)' $conf ||
    {
      echo "ERROR: VPN CA cert missing!"
      sleep 120
    }
  set -x
